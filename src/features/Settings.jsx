...
import {
  ...
  Typography, Card, CardContent, Divider, Chip, Box, Alert,
} from "@mui/material";
...
function Section({ icon, title, hint, children }) {
  return (
    <Card sx={{ bgcolor: "action.hover" }}>
      <CardContent>
        <Typography variant="h3" sx={{ display: "flex", alignItems: "center", gap: 1, mb: 1.5 }}>
          <span>{icon}</span> {title}
          {hint && <Typography variant="caption" color="text.secondary" sx={{ ml: "auto" }}>{hint}</Typography>}
        </Typography>
...
export default function Settings({ open, onClose }) {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="sm" scroll="paper">
      ...
      <DialogContent>
        <Chip color="primary" label={`Angemeldet als ${me.name}${me.is_admin ? " (Admin)" : ""}`} sx={{ mb: 2 }} />
        <Stack spacing={1.5}>
          <Section icon="🔒" title="Passwort & Nutzer">
            <TextField label="Mein neues Passwort" type="password" value={pwNew} onChange={(e) => setPwNew(e.target.value)} />
            <Button variant="contained" disabled={pwNew.length < 4 || changePw.isPending}
              onClick={() => changePw.mutate(pwNew, { onSuccess: () => { setPwNew(""); alert("Passwort gespeichert."); } })}>
              Passwort speichern
            </Button>
            {me.is_admin && (
              <>
                <Divider />
                <Typography variant="body2" color="text.secondary">Admin: Passwort für {otherUser} setzen</Typography>
                <TextField label={`Neues Passwort für ${otherUser}`} type="password" value={otherPw} onChange={(e) => setOtherPw(e.target.value)} />
                <Button variant="outlined" disabled={otherPw.length < 4 || adminPw.isPending}
                  onClick={() => adminPw.mutate({ name: otherUser, pw: otherPw }, { onSuccess: () => { setOtherPw(""); alert("Gesetzt."); } })}>
                  Für {otherUser} setzen
                </Button>
              </>
            )}
          </Section>

          <Section icon="🔔" title="Zahlungs-Erinnerung">
            <TextField label="Erinnern am … Tag des Monats" type="number" value={remDay}
              onChange={(e) => setRemDay(e.target.value)} inputProps={{ min: 1, max: 28 }} sx={{ maxWidth: 220 }} />
          </Section>

          <Section icon="🏦" title="Bankverbindungen" hint="für GiroCode">
          ...
          <Section icon="🤖" title="KI-Assistent" hint="Chat · Scan · Diktat">
          ...
          <Button color="error" onClick={() => logout.mutate()}>Abmelden</Button>
          {saveSettings.isError && <Alert severity="error">{String(saveSettings.error.message)}</Alert>}
        </Stack>
      </DialogContent>
      <DialogActions sx={{ px: 3, pb: 2, position: "sticky", bottom: 0, bgcolor: "background.paper" }}>
